...
#if USE_EF
            builder.Configuration.AddInMemoryCollection(builder.Configuration.KeyValueDbSetAndGenericApi(
                typeof(GenericApiController<,>),
                (typeof(Todo), typeof(int), nameof(Todo.Id)),
                (typeof(Kanban), typeof(int), nameof(Kanban.Id))));
            builder.Configuration.AddInMemoryCollection(builder.Configuration.KeyValueDbSetAndGenericApi(
                typeof(GenericReadOnlyController<,>), 
                (typeof(Todo), typeof(int), nameof(Todo.Id))));
