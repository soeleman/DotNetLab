...
namespace Microsoft.AspNetCore.Builder
{
    public abstract class StorageBase<T, TK>
    {
        ...
        protected (bool state, PropertyInfo? propInfo) GetKeyProperty(
            TK data,
            string keyName = "Id")
        {
            if (data == null)
            ...
            return dataType
                .GetProperties()
                .Any(p => p.Name.Equals(keyName, StringComparison.InvariantCultureIgnoreCase))
                    ? (true, dataType.GetProperty(KeyMap))
                    : (false, null);
        }

        protected (T key, TK nextIdData) AutoId(
            TK newData)
        {
            if (newData == null)
            {
            ...
            string[] excludeProperty)
        {
            ...
            {
                foreach (var toProperty in currentData
                    .GetType()
                    .GetProperties())
                {
                    ...
                    toProperty.SetValue(
                        currentData,
                        fromProperty.GetValue(updateData));
